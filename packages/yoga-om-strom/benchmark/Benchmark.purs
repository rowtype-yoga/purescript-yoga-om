...
import Effect.Aff (Aff, launchAff_, Fiber, forkAff, joinFiber)
...
runOm om = Om.runOm {} { exception: \_ -> pure unit } om
...
main = launchAff_ do
  ...
  timeOperation "Map 1M elements" do
    start <- liftEffect now
    result <- runOm $
  ...
  -- Benchmark 2: Filter
  timeOperation "Filter 1M elements (50% kept)" do
    start <- liftEffect now
    result <- runOm $
      Strom.rangeStrom 1 1000001
        # Strom.filterStrom (\n -> n `mod` 2 == 0)
  ...
  timeOperation "Chain 3 maps over 1M elements" do
    start <- liftEffect now
    result <- runOm $
      Strom.rangeStrom 1 1000001
        # Strom.mapStrom (_ + 1)
  ...
  timeOperation "Complex pipeline (map, filter, map) on 1M elements" do
    start <- liftEffect now
    result <- runOm $
      Strom.rangeStrom 1 1000001
        # Strom.mapStrom (_ + 1)
  ...
  timeOperation "Fold 1M elements (sum)" do
    start <- liftEffect now
    result <- runOm $
      Strom.rangeStrom 1 1000001
        # Strom.runFold (\acc n -> acc + n) 0
    end <- liftEffect now
    let Milliseconds duration = diff end start
  ...
  timeOperation "Collect 100K elements" do
    start <- liftEffect now
    result <- runOm $
  ...
  timeOperation "Take 10K from 1M elements" do
    start <- liftEffect now
    result <- runOm $
      Strom.rangeStrom 1 1000001
  ...
  timeOperation "Non-deterministic merge of 2x500K elements" do
    start <- liftEffect now
    result <- runOm $
      Strom.mergeND
  ...
  timeOperation "Parallel map (concurrency=4) on 1000 elements" do
    start <- liftEffect now
    result <- runOm $
  ...
  timeOperation "Infinite stream, take 1M elements" do
    start <- liftEffect now
    result <- runOm $
  ...
  timeOperation "Scan (running sum) over 1M elements" do
    start <- liftEffect now
    result <- runOm $
      Strom.rangeStrom 1 1000001
  ...
  -- Benchmark 12: Unfold
  timeOperation "Unfold 100K elements" do
    start <- liftEffect now
    result <- runOm $
      Strom.unfoldStrom (\n -> if n > 100000 then Nothing else Just { head: n, tail: n + 1 }) 1
        # Strom.runDrain
    end <- liftEffect now
    let Milliseconds duration = diff end start
